...
import com.groops.fairsquare.utility.FileFromUriListener;
import com.groops.fairsquare.utility.FileFromUriTask;
import com.groops.fairsquare.utility.GlideApp;
import com.groops.fairsquare.utility.LogUtils;
import com.groops.fairsquare.utility.ProgressRequestBody;
...
//                      .apply(RequestOptions.onlyRetrieveFromCache(true)
	...
	public static void loadProfilePictureWithPlaceholder(Context context, String url, ImageView target) {
		loadProfilePictureWithPlaceholder(context, url, target, false);
	}
	 ...
	 * @param target the {@link ImageView} which is target container for the image.
	 */
	public static void loadProfilePictureWithPlaceholder(Context context, String url, ImageView target, boolean forInterest) {
		loadPictureWithGlide(
				context,
				url,
				null,
				forInterest ? R.drawable.ic_interest_placeholder : R.drawable.ic_profile_placeholder,
				forInterest ? R.drawable.ic_interest_placeholder : R.drawable.ic_profile_placeholder,
				target
		);
	}
...
//		intent.setType(MIME_PNG);           // image PNG
	...
	public static class UploadService {
		 ...
		 * @param context  the application's/activity's {@link Context} instance.
		 * @param file     the file to be uploaded.
		 * @param userId   the {@link com.groops.fairsquare.models.HLUser#_id} of the user uploading the picture.
		 * @param listener the callback receiver when uploading is concluded.
		 * @return True if the operation is successful.
		 * @throws IOException if the operation is not successful.
		 * @throws IllegalArgumentException if some of or all the provided pieces are invalid.
			...
			if (Utils.areStringsValid(mediaType, format, file.getName())) {
				...
				try {
					final RequestBody requestBody = new MultipartBody.Builder().setType(MultipartBody.FORM)
							.addFormDataPart("file", file.getName(), new ProgressRequestBody(file, mediaType, new ProgressRequestBody.ProgressListener() {
								@Override
