...
import com.groops.fairsquare.activities_and_fragments.activities_chat.ChatMessagesFragment
import com.groops.fairsquare.activities_and_fragments.activities_chat.ChatRoomsFragment
...
import com.groops.fairsquare.base.BaseSingletonWithArgument
import com.groops.fairsquare.base.HLActivity
import com.groops.fairsquare.base.HLApp
import com.groops.fairsquare.models.*
...
import com.groops.fairsquare.notifications.FCMMessagingService
import com.groops.fairsquare.services.BaseHandlePushedBlockService
import com.groops.fairsquare.services.HandlePushedChannelsService
import com.groops.fairsquare.services.HandlePushedDataService
...
class RealTimeCommunicationHelperKotlin private constructor(private var mContext: Context) :
    ...
    private fun onFeedChannelsPushed(jsonArray: JSONArray?) {
        if (jsonArray != null)
            HandlePushedChannelsService.startService(mContext, mHandler, jsonArray.toString())
    }
        ...
        if (json != null) {
            ...
            mRealm?.executeTransaction {
                ...
                if (roomPair.first) {
                    ...
                    // INFO: 4/2/19    Assigns correct ownerID (if message received it cannot be the user's) and Fixes wrong call to Room initialization onNewMessage received
            ...
            if (listenerChat != null) {
                when (listenerChat) {
                    is ChatRoomsFragment -> {
                        ...
                        if (HLApp.canVibrate) vibrateForChat(mContext)
    ...
    private fun onStatusUpdated(json: JSONObject?) {
        if (json != null) {
            ...
            val status = json.optInt("status", ChatParticipant.Status.OFFLINE.value)
            val date = json.optString("date", "")

            val user = RealmUtils.readFirstFromRealmWithId(mRealm, ChatParticipant::class.java, "userID", userId) as? ChatParticipant
            mRealm?.executeTransaction {
    ...
    override fun handleSuccessResponse(operationId: Int, responseObject: JSONArray?) {
        ...
        try {
            ...
            when (operationId) {
                ...
                Constants.SERVER_OP_RT_PUSH_CHANNELS -> onFeedChannelsPushed(responseObject)
...
//        }
    ...
    override fun handleMessage(msg: Message?): Boolean {
        ...
        when (msg?.what) {
            ...
            BaseHandlePushedBlockService.Type.CHANNELS.value -> {
                if (listener != null && listener is TimelineFragment) {
                    (listener as TimelineFragment).onChannelUpdatesPushed()
                    return true
                }
            }
    ...
    private fun callSetDataRead() {
        ...
        if (mContext is HLActivity)
            HLRequestTracker.getInstance((mContext as HLActivity).application as HLApp)
