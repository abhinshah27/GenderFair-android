...
const val MEDIA_UPLOAD_NEW_PIC_PROFILE_SIGNUP = "pps"
...
class MediaUploadManager(context: Context,
    ...
    inner class UploadService(private val activity: HLActivity) {
         ...
         * @param userId   the [com.groops.fairsquare.models.HLUser._id] of the user uploading the picture.
    ...
    override fun onProcessingComplete(pair: Pair<File?, Int>) {
        ...
        // INFO: 2019-08-07    user?.id -> user?.userId because currently used only by chat
        uploadMedia(
                contextRef.get(),
                file = pair.first,
