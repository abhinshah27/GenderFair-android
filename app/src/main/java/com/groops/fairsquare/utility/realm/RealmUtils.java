...
package com.groops.fairsquare.utility.realm;
...
public class RealmUtils {
	...
	synchronized public static void closeRealm(Realm realm) {
		if (realm != null) {
			realm.close();
			HLApp.openRealms.remove(realm);
		}
	}

	synchronized public static void closeAllRealms() {
		LogUtils.d("REALM", "Inside closeAllRealms()");

		closingAllRealms = true;

		if (HLApp.openRealms != null && !HLApp.openRealms.isEmpty()) {
			for (Realm realm : HLApp.openRealms) {
				try {
	...
	public static void setClosingAllRealms(boolean closingAllRealms) {
		RealmUtils.closingAllRealms = closingAllRealms;
	}
