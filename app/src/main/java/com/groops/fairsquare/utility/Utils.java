...
import com.groops.fairsquare.activities_and_fragments.SelectIdentityActivity;
import com.groops.fairsquare.activities_and_fragments.WebViewActivity;
import com.groops.fairsquare.activities_and_fragments.WebViewActivityDocuments;
...
import com.groops.fairsquare.services.FCMNotNeededException;
...
public class Utils {
	...
	public static int getToolbarHeight(Context context) {
		...
		int toolbar = a.getDimensionPixelSize(0, dpToPx(56f, context.getResources()));
		a.recycle();
...
//							.setDismiss(true)
	 ...
	 */
	public static boolean hasApplicationPermission(Activity context, String permission) {
...
//								LogUtils.i(context.getClass().getCanonicalName(), "Refreshed FCM token: " + refreshedToken);
								...
								if (!HLApp.fcmTokenSent || (Utils.isStringValid(refreshedToken) && !fcm.equals(refreshedToken))) {
	 ...
	 */
	public static void logOut(Activity activity, @Nullable MaterialDialog dialog, @Nullable String messageError) {
		...
		SessionWrapper.resetSession();

		SessionHandler.INSTANCE.clearPending();
		SessionHandler.INSTANCE.setLogoutProgressing(false);

		// Realm instances to be closed now should always be created on UIThread
		activity.runOnUiThread(() -> {
            RealTimeCommunicationHelperKotlin.Companion.getInstance(activity).closeRealmInstance();
            RealmUtils.closeAllRealms();

            try {
	            Realm.deleteRealm(HLApp.realmConfig);
			} catch (Exception e) {
            	e.printStackTrace();
            }

            RealmUtils.setClosingAllRealms(false);
		...
		if (Utils.isStringValid(messageError)) {
			// INFO: 2019-05-30    EXTRA_PARAM_1 used for redirection from email
			intent.putExtra(Constants.EXTRA_PARAM_2, messageError);
		}
	...
	public static boolean checkAndOpenLogin(Activity activity, HLUser user) {
