...
import com.groops.fairsquare.voiceVideoCalls.model.IncomingCall
            ...
            if (response.isSuccessful) {
                ...
                if (contextRef?.get() is Activity) {
                    (contextRef?.get() as Activity).runOnUiThread {
                        Toast.makeText(contextRef?.get(), "Error retrieving token", Toast.LENGTH_SHORT).show()
                    }
                }
            }
...
object NotificationUtils {
    ...
    fun sendCallNotificationAndGoToActivity(context: HLActivity, callerID: String,
                                            callerName: String, calledIdentityUser: HLUserGeneric? = null,
                                            calledIdentityParticipant: ChatParticipant? = null,
                                            callType: VoiceVideoCallType = VoiceVideoCallType.VOICE) {

        if (calledIdentityUser == null && calledIdentityParticipant == null) {
            context.runOnUiThread { context.showAlert(context.getString(R.string.error_calls_start)) }
            return
        }

        val calledId = calledIdentityUser?.id ?: calledIdentityParticipant!!.userID
        val calledName = calledIdentityUser?.completeName ?: calledIdentityParticipant!!.name
        val calledAvatar = calledIdentityUser?.avatarURL ?: calledIdentityParticipant!!.avatarURL
        val calledWall = calledIdentityUser?.wallImageLink ?: calledIdentityParticipant!!.userID
