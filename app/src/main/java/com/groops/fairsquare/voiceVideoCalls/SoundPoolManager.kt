...
import com.groops.fairsquare.utility.BaseSoundPoolManager
import com.groops.fairsquare.utility.Utils
import com.groops.fairsquare.utility.vibrateForCalls
...
class SoundPoolManager private constructor(context: Context,
    ...
    fun playRinging(context: Context) {
        if (usageType == VoiceVideoUsageType.OUTGOING) {
            ...
            if (HLApp.canPlaySounds && mediaPlayer != null) {
                mediaPlayer!!.start()
                playingRingtone = true
            }

            if (HLApp.canVibrate) vibrateForCalls(context)
        }
    }
