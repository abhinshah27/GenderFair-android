...
public class FetchingOperationsService extends Service implements OnServerMessageReceivedListener {
	...
	private static final int NUM_OPS = 3;       // INTEREST postponed
...
//		registerReceiver(receiver, new IntentFilter(Constants.BROADCAST_SERVER_RESPONSE));
		...
		try {
			...
			callServer(Operations.CHANNELS, realm);
//			callServer(Operations.INTERESTS, realm);
	...
	private enum Operations { IDENTITIES, CIRCLES, INTERESTS, CHANNELS }
	...
	private void callServer(Operations op, Realm realm) {
		...
		try {
			...
			else if (op == Operations.CHANNELS)
				results = HLServerCalls.getChannels();
		...
		switch (operationId) {
			case Constants.SERVER_OP_GET_IDENTITIES_V2:
				...
				try {
					...
					realm.executeTransaction(realm13 -> user.setIdentities(responseObject));
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					RealmUtils.closeRealm(realm);
			...
			case Constants.SERVER_OP_GET_MY_INTERESTS:
				...
				try {
					realm = RealmUtils.getCheckedRealm();
					final HLUser user = new HLUser().readUser(realm);
					realm.executeTransaction(realm12 -> user.setIdentities(responseObject));
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					RealmUtils.closeRealm(realm);
