...
package com.groops.fairsquare.notifications;
...
//		resultIntent.putExtra("some code", true);
	...
	private boolean checkFcmToken(Context context, String newToken) {
		...
		if (!HLApp.fcmTokenSent || (Utils.isStringValid(newToken) && !fcm.equals(newToken))) {
	...
	private void sendNotification(int id, String msg, Intent extra) {
		...
		if (mNotificationManager != null) {
			...
			if (extra == null) {
				// Class selection logic maintained BUT it should always be SplashActivity, since SOCIAL notifs are not supposed to be sent when app is in foreground.
				extra = new Intent(this, HLApp.isForeground ? ProfileActivity.class : SplashActivity.class);
...
//			int _id = (!Utils.isStringValid(id) ? (int) when : id.hashCode());
	...
	public static void createChatChannelAndSendNotification(Context context, int id, String msg, Intent extra) {
		...
		if (mNotificationManager != null) {
			...
			if (extra == null) {
				extra = new Intent(context, HLApp.isForeground ? HomeActivity.class : SplashActivity.class);
				extra.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
            ...
            StringBuilder realMsg = new StringBuilder();
            if (parts.length == 2) realMsg = new StringBuilder(parts[1]);
            else if (parts.length > 2) {
				for (int i = 1; i < parts.length; i++) {
					if (i == 1) realMsg.append(parts[i]);
					else realMsg.append(":").append(parts[i]);
				}
			}
            ...
            if (/*wantsGroup = */(Utils.hasNougat() && /*parts.length == 2 && */Utils.areStringsValid(parts[0], realMsg.toString()))) {
                if (Utils.hasPie())
                    style = new NotificationCompat.MessagingStyle(new Person.Builder().setName("Me").build())
                            .addMessage(realMsg, when, new Person.Builder().setName(parts[0]).build());
                else
                    style = new NotificationCompat.MessagingStyle("Me")
                            .addMessage(new NotificationCompat.MessagingStyle.Message(realMsg, when, parts[0]));
            }
