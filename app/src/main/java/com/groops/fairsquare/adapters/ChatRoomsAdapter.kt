...
import com.groops.fairsquare.utility.isSameDateAsNow
...
class ChatRoomsAdapter(
    ...
    fun filterResults(text: String) {
        val query = fragment.getRealm()
                .where(ChatRoom::class.java)
                .equalTo("identityID", fragment.getUser().userId)
        ...
        fun setRoom(currentRoom: ChatRoom?, position: Int) {
            if (currentRoom != null) {
                ...
                chatRoomAvail.visibility =
                        if (currentRoom.recipientStatus == ChatParticipant.Status.ONLINE.value)
                            View.VISIBLE
                        else View.GONE
