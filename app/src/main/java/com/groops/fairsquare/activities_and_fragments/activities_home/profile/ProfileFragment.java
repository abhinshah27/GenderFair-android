...
import com.groops.fairsquare.models.HLInterests;
...
//								mUser = RealmUtils.checkAndFetchUser(realm, mUser);
			...
			case Constants.SERVER_OP_GET_PERSON_V2:
				...
				// INFO: 2019-06-20    CALLS ARE DISABLED
	...
	public void onDotsClick(View dots, final View preferredLabel) {
		if (profileType == ProfileHelper.ProfileType.INTEREST_CLAIMED ||
			...
			if (interest != null) {
				...
				claim.setVisible(!interest.isClaimed());
				menu.show();
			}
		}
	}
...
//		}
	...
	public void goToInterestDetails() {
		if (profileType == ProfileHelper.ProfileType.INTEREST_ME)
			ProfileActivity.openInterestWebviewFragment(getContext(), InterestProfileFragmentWebview.Type.DETAIL,
					interest.getId(), interest.getAvatarURL(), interest.getWallPictureURL(), interest.getScore());
		else if (interest != null)
			profileActivityListener.showInterestWebviewFragment(InterestProfileFragmentWebview.Type.DETAIL,
					interest.getId(), interest.getAvatarURL(), interest.getWallPictureURL(), interest.getScore());
	}
	...
	@Override
	public void goToBrands(String companyID, String name, String avatarUrl, String wallPicture, String score) {
		if (getActivity() instanceof HomeActivity)
			ProfileActivity.openInterestBrandsFragment(getContext(), companyID, name, avatarUrl, wallPicture, score);
		else if (getActivity() instanceof ProfileActivity)
			profileActivityListener.showInterestBrandsFragment(companyID, name, avatarUrl, wallPicture, score);
	}

	@Override
	public void goToInterestWebview(String companyID, String name, String avatarUrl, String wallPicture, String score) {
		if (getActivity() instanceof HomeActivity)
			ProfileActivity.openInterestWebviewFragment(getContext(), InterestProfileFragmentWebview.Type.PRODUCTS,
					companyID, avatarUrl, wallPicture, score);
		else if (getActivity() instanceof ProfileActivity)
			profileActivityListener.showInterestWebviewFragment(InterestProfileFragmentWebview.Type.PRODUCTS,
					companyID, avatarUrl, wallPicture, score);
	}

	@Override
	public void goToCompany(String companyID) {
		if (getActivity() instanceof HomeActivity)
			ProfileActivity.openProfileCardFragment(getContext(), ProfileHelper.ProfileType.INTEREST_NOT_CLAIMED, companyID, -1);
		else if (getActivity() instanceof ProfileActivity)
			profileActivityListener.showProfileCardFragment(ProfileHelper.ProfileType.INTEREST_NOT_CLAIMED, companyID);
	}
	...
	public void clearImageResources() {
		if (profileHelper != null)
			profileHelper.clearImageResources();
	}
	...
	public View getTopIconNotificationDot() {
		if (profileHelper != null)
			return profileHelper.getTopIconNotificationDot();
