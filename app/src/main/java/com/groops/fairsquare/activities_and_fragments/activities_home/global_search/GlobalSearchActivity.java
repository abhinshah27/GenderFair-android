...
//		NotificationAndRequestHelper.handleDotVisibility(notificationDot1, mUser.isValid());
		...
		boolean condition = mUser.isValid() && (ChatRoom.Companion.getAmountUnreadMessagesByIdentityID(mUser.getId(), realm) > 0);
	...
	protected void manageIntent() {
		...
		switch (showFragment) {
			case Constants.FRAGMENT_GLOBAL_SEARCH:
				addGlobalSearchFragment(null, requestCode, false);
				break;
...
//		}
	...
	public static void openGlobalSearchFragment(Context context) {
		FragmentsUtils.openFragment(context, null, Constants.FRAGMENT_GLOBAL_SEARCH, Constants.NO_RESULT,
				GlobalSearchActivity.class);
	}

	private void addGlobalSearchFragment(Fragment target, int requestCode, boolean animate) {
		...
		if (fragment == null) {
			fragment = GlobalSearchFragment.newInstance();
