...
import com.groops.fairsquare.activities_and_fragments.activities_user_guide.UserGuideActivity;
	...
	protected void manageIntent() {
		...
		switch (showFragment) {
			...
			case Constants.FRAGMENT_SETTINGS_HELP:
				...
				addSettingsHelpFragment(null, new SettingsHelpElement(), requestCode, false);
				break;
...
//			bottomBar.setVisibility(visible ? View.VISIBLE : View.GONE);
	...
	public void refreshProfilePicture(@NonNull String link) {
		MediaHelper.loadProfilePictureWithPlaceholder(this, link, profilePicture, mUser.isActingAsInterest());
	}
	...
	public void showInnerCircleCirclesFragment() {
		addSettingsCirclesFoldersFragment(null, SettingsCirclesFoldersFragment.ViewType.CIRCLES, Constants.NO_RESULT, true);
	}

	private void addSettingsCirclesFoldersFragment(Fragment target, SettingsCirclesFoldersFragment.ViewType type,
		...
		SettingsCirclesFoldersFragment fragment = (SettingsCirclesFoldersFragment) fragmentManager.findFragmentByTag(SettingsCirclesFoldersFragment.LOG_TAG);
//		if (fragment == null) {
		fragment = SettingsCirclesFoldersFragment.newInstance(type);
		FragmentsUtils.addFragmentNull(fragmentTransaction, R.id.pages_container, fragment,
				SettingsCirclesFoldersFragment.LOG_TAG, target, requestCode);
//		} else
//			FragmentsUtils.addFragmentNotNull(fragmentTransaction, fragment, target, requestCode);
		fragmentTransaction.commit();
	}
	...
	@Override
	public void showInnerCircleSingleCircleFragment(@NonNull String circleName, @Nullable String filter) {
		addSettingsInnerCircleSingleCircleFragment(null, circleName, filter, Constants.NO_RESULT, true);
	}

	private void addSettingsInnerCircleSingleCircleFragment(Fragment target, @NonNull String circleName,
...
//		if (fragment == null) {
		fragment = SettingsICSingleCircleFragment.newInstance(circleName, filter);
		FragmentsUtils.addFragmentNull(fragmentTransaction, R.id.pages_container, fragment,
...
//			FragmentsUtils.addFragmentNotNull(fragmentTransaction, fragment, target, requestCode);
	...
	private void addSettingsFoldersFragment(Fragment target, int requestCode, boolean animate) {
		addSettingsCirclesFoldersFragment(target, SettingsCirclesFoldersFragment.ViewType.FOLDERS, requestCode, animate);
	}
...
//			FragmentsUtils.addFragmentNotNull(fragmentTransaction, fragment, target, requestCode);
	...
	@Override
	public void showSettingsHelpListFragment(@NonNull SettingsHelpElement element) {
		addSettingsHelpFragment(null, element, Constants.NO_RESULT, true);
	}

	private void addSettingsHelpFragment(Fragment target, @NonNull SettingsHelpElement element, int requestCode, boolean animate) {
		FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
		if(animate)
			fragmentTransaction.setCustomAnimations(R.anim.slide_in_right, R.anim.slide_out_left,
					R.anim.slide_in_left, R.anim.slide_out_right);
...
//		if (fragment == null) {
		fragment = SettingsHelpListFragment.newInstance(element);
		FragmentsUtils.addFragmentNull(fragmentTransaction, R.id.pages_container, fragment,
