...
import com.groops.fairsquare.models.enums.*
import com.groops.fairsquare.utility.*
import com.groops.fairsquare.utility.helpers.HLMediaType
import com.groops.fairsquare.utility.helpers.LIMIT_MIN_SIZE_FOR_UPLOAD_VIDEO
...
class CreatePostPreviewActivity: HLActivity(), OnServerMessageReceivedListener, OnMissingConnectionListener,
    ...
    private fun initLayout() {
        ...
        if (mediaCaptureType == HLMediaType.AUDIO) {
            if (HLApp.siriComposition != null)
                waveAnimation.setComposition(HLApp.siriComposition)
            else {
                HLApp.initLottieAnimation(this, object : LottieCompositioListener() {
    ...
    private fun setLayout() {
        ...
        MediaHelper.loadProfilePictureWithPlaceholder(this, mUser.avatarURL, profilePicture.pic as ImageView, mUser.isActingAsInterest)
    ...
    private fun attemptSendingPost(fromWiFiDialog: Boolean) {
        ...
        if (!checkConnectionAndShow(fromWiFiDialog)) {
            ...
            HLRequestTracker.getInstance(application as HLApp).handleCallResult(this, this, results)
        }
    }
