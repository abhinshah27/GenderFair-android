...
import com.groops.fairsquare.activities_and_fragments.activities_home.OnActionsResultFromBottom;
import com.groops.fairsquare.activities_and_fragments.activities_home.global_search.GlobalSearchActivity;
...
import com.groops.fairsquare.adapters.ChannelsAdapter;
import com.groops.fairsquare.adapters.FeedMemoryViewHolder;
import com.groops.fairsquare.adapters.InnerCircleFeedAdapter;
...
import com.groops.fairsquare.models.FeedFilter;
...
import com.groops.fairsquare.models.enums.FeedFilterTypeEnum;
...
import com.groops.fairsquare.utility.helpers.PostBottomSheetHelper;
import com.groops.fairsquare.utility.helpers.RealTimeCommunicationHelperKotlin;
import com.groops.fairsquare.utility.helpers.RealTimeCommunicationListener;
...
public class TimelineFragment extends HLFragment implements View.OnClickListener,
		...
		ChannelsAdapter.OnChannelSelectedListener {
	...
	private TextView /*sortingSection,*/ sortRecentTxt, sortLovedTxt, sortShuffleTxt;
//	private ImageView sortBtn;
	...
	private ViewGroup filterContainer, sortContainer;
	...
	private View channelsClickConsumer;
	private boolean channelsVisible = false;
	private RecyclerView feedChannels;
	private LinearLayoutManager channelsLlm;
	private ChannelsAdapter channelsAdapter;
	private List<FeedChannel> channelsList = new ArrayList<>();

	private ObjectAnimator animationChannelsOn = null, animationChannelsOff = null;
...
//			((HomeActivity) getActivity()).setPostDeletedListener(this);
		...
		else if (getActivity() instanceof SinglePostActivity) {
			((SinglePostActivity) getActivity()).setBackListener(this);
		}
		...
		*/
		...
		if (isTypeTimeline()) handleChannelsAdapter();
...
//		toolbar.findViewById(R.id.sort_click_consumer).setOnClickListener(this);
		...
		channelsClickConsumer = toolbar.findViewById(R.id.channels_click_consumer);
		channelsClickConsumer.setOnClickListener(this);
		...
		if (isTypeTimeline()) {
			...
			toolbarDiarySingle.findViewById(R.id.back_arrow).setOnClickListener(this);
		}
...
//				getContext().unregisterReceiver(filterRcvr);
	...
	public void onDetach() {
		...
		if (Utils.isContextValid(getActivity())) {
			if (getActivity() instanceof HomeActivity &&
                    ((HomeActivity) getActivity()).getBackListener() instanceof TimelineFragment) {
                ((HomeActivity) getActivity()).setBackListener(null);
            }
			else if (getActivity() instanceof GlobalSearchActivity &&
                    ((GlobalSearchActivity) getActivity()).getBackListener() instanceof TimelineFragment)
	...
	public void onClick(View view) {
		...
		if (feedFilter != null && feedFilter.getVisibility() == View.VISIBLE) {
			switch (id) {
				...
				case R.id.sort_shuffle:
					handleSorting(view);
					break;
				default: handleFilterToggle();
			}
		}
		else if (channelsVisible) {
			...
			switch (id) {
				...
				case R.id.channels_click_consumer:
					handleChannelsVisibility();
					break;
	...
	private void onRestoreInstanceState(Bundle arguments, Bundle savedInstanceState) {
		...
		if (filtersString != null && !filtersString.isEmpty()) {
			HLApp.refreshAllowed = !filtersString.containsAll(mUser.getSelectedFeedFilters()) ||
					!mUser.getSelectedFeedFilters().containsAll(filtersString);

			if (HLApp.refreshAllowed)
	...
	public void onBackPressed() {
		...
		boolean sheetUp;
		if (bottomSheetHelper == null) sheetUp = !mListener.closePostSheet();
		else sheetUp = !bottomSheetHelper.closePostSheet();
		...
		if (sheetUp) {
			if (Utils.isContextValid(getActivity())) {
				if (getActivity() instanceof HomeActivity) {
					...
					((SinglePostActivity) getActivity()).setBackListener(null);
					getActivity().onBackPressed();
				}
				else if (getActivity() instanceof SettingsActivity) {
	...
	public void handleSuccessResponse(int operationId, JSONArray responseObject) {
		...
		if (Utils.isContextValid(context)) {
			switch (operationId) {
				...
				case Constants.SERVER_OP_GET_TIMELINE_CHANNEL:
...
//									.execute(responseObject);
						...
						boolean condition = ((operationId == Constants.SERVER_OP_GET_TIMELINE) || (operationId == Constants.SERVER_OP_GET_TIMELINE_CHANNEL)) &&
				...
				case Constants.SERVER_OP_GET_CHANNELS:

					GFChannels.INSTANCE.setChannels(responseObject, true);
					setChannels();
	...
	//region == POST SHEET HELPER actions ==
...
//		indexBeforeDeletion = posts.getPostIndex(postId, false);
	...
	@Override
	public void onChannelUpdatesPushed() {
		if (isTypeTimeline() && hasChannelsVisible()) setChannels();
	}
...
//				LocalBroadcastManager.getInstance(getContext()).unregisterReceiver(realTimeHelper.getServerMessageReceiver());
	...
	protected void setLayout() {
		...
		toolbar.setVisibility(canShowToolbarForFullScreen && isTypeTimeline() ? View.VISIBLE : View.GONE);
		toolbarDiarySingle.setVisibility(!canShowToolbarForFullScreen || isTypeTimeline() ? View.GONE : View.VISIBLE);

		// INFO: 4/16/19    channels visible even if INTEREST is acting
//		channelsClickConsumer.setVisibility(mUser.isActingAsInterest() ? View.GONE : View.VISIBLE);

		int px = Utils.getToolbarHeight(getContext());
...
//		sortBtn.setImageResource(sortResId);
			...
			filterToggle.setVisibility(isTypeTimeline() ? View.VISIBLE : View.GONE);
	...
	private boolean isTypeTimeline() {
	...
	private void saveLastPostSeen(boolean reset) {
		if (isTypeTimeline()) {
	...
	/**
	 * General method calling Timeline data.
	 *
	 * @param explicitSortOrder The optional explicit sorting order.
	 * @param channelID The optional selected channel ID.
	 * @param wantsTop Whether a reset of the pagination skip is required.
	 */
	private void callTimeline(Integer explicitSortOrder, String channelID, boolean wantsTop) {

		Object[] results = null;
		try {

		    int skip = mListener.getFeedSkip(realm, FragmentUsageType.TIMELINE, wantsTop);
			...
			if (!Utils.isStringValid(channelID)) {
				results = HLServerCalls.getTimeline(mUser.getId(), mUser.getCompleteName(), mUser.getAvatarURL(),
						...
						skip, filtersString, mUser.isActingAsInterest());
			}
			else {
				String name = GFChannels.INSTANCE.getChannelNameByID(channelID);
				results = HLServerCalls.getTimelineForChannel(channelID, name, skip);
			}
		} catch (JSONException e) {
			e.printStackTrace();
		}
	...
	/**
	 * Method calling Timeline data giving value to sorting order property and nullifying to User's "selectedChannel".
	 *
	 * @param explicitSortOrder The explicitly required sorting order.
	 * @param wantsTop Whether a reset of the pagination skip is required.
	 */
	private void callTimeline(Integer explicitSortOrder, boolean wantsTop) {
		callTimeline(explicitSortOrder, null, wantsTop);
	}

	/**
	 * Method calling Timeline data giving value to User's "selectedChannel".
	 *
	 * @param wantsTop Whether a reset of the pagination skip is required.
	 */
	private void callTimeline(boolean wantsTop) {
		callTimeline(null, GFChannels.INSTANCE.getCurrentSelectedChannel(), wantsTop);
	}
	...
	private void callChannels() {
		...
		try {
			if (mUser.isValid())
				result = HLServerCalls.getChannels();
		} catch (JSONException e) {
			e.printStackTrace();
		}
...
//			else {
			...
			if (sortRecent.isSelected())
				selectedSortOrder = Constants.SERVER_SORT_TL_ENUM_RECENT;
			else if (sortLoved.isSelected())
				selectedSortOrder = Constants.SERVER_SORT_TL_ENUM_LOVED;
			else if (sortShuffle.isSelected())
				selectedSortOrder = Constants.SERVER_SORT_TL_ENUM_SHUFFLE;

			// resets selected channel to null
			GFChannels.INSTANCE.setCurrentSelectedChannel(null);
...
//			}
		...
		sortLoved = view.findViewById(R.id.sort_loved);
		sortLoved.setOnClickListener(this);
		sortLovedTxt = sortLoved.findViewById(R.id.text);
		sortRecent = view.findViewById(R.id.sort_date);
		sortRecent.setOnClickListener(this);
		sortRecentTxt = sortRecent.findViewById(R.id.text);
		sortShuffle = view.findViewById(R.id.sort_shuffle);
		sortShuffle.setOnClickListener(this);
		sortShuffleTxt = sortShuffle.findViewById(R.id.text);

	}

	private void populateFilters(Realm realm, HLUser mUser) {
		if (filters == null)
		...
		if (Utils.isContextValid(getContext())) {
			if (!mUser.isActingAsInterest()) {
				...
				List<HLCircle> circles = new ArrayList<>(mUser.getCircleObjects());
				if (circles.size() > 0) {
					Collections.sort(circles, HLCircle.CircleNameComparator);
					for (HLCircle circle : circles) {
						filters.add(new FeedFilter(circle.getName()));
					}
				}
	...
	private void setFilterView(Realm realm, HLUser mUser) {
		if (filterContainer != null) {
			...
			if (filters != null && !filters.isEmpty()) {
				...
				final boolean isInterest = mUser.isActingAsInterest();
				...
				for (final FeedFilter filter : filters) {
					if (filter != null) {
						...
						if (view != null) {
							((TextView) view.findViewById(R.id.text)).setText(filter.getName());
		...
		if (sortRecentTxt != null) sortRecentTxt.setText(R.string.settings_ic_sort_recent);
		if (sortLovedTxt != null) sortLovedTxt.setText(R.string.settings_ic_sort_loved);
		if (sortShuffleTxt != null) sortShuffleTxt.setText(R.string.settings_ic_sort_shuffle);
	}


	private void configureChannels(View view) {
		feedChannels = view.findViewById(R.id.channels_list);
	}

	public void handleChannelsVisibility() {
		channelsVisible = !channelsVisible;
		if (channelsVisible) setChannels();

		if (channelsVisible) {
			if (animationChannelsOn == null) {
				animationChannelsOn = ObjectAnimator.ofFloat(feedChannels, "translationY", Utils.getToolbarHeight(getContext()));
				animationChannelsOn.addListener(new Animator.AnimatorListener() {
					@Override
					public void onAnimationStart(Animator animation) {
						feedChannels.setVisibility(View.VISIBLE);
					}
			...
			animationChannelsOn.start();
		} else {
			if (animationChannelsOff == null) {
				animationChannelsOff = ObjectAnimator.ofFloat(feedChannels, "translationY", 0);
				animationChannelsOff.addListener(new Animator.AnimatorListener() {
					@Override
					public void onAnimationStart(Animator animation) {}

					@Override
					public void onAnimationEnd(Animator animation) {
						feedChannels.setVisibility(View.GONE);
					}
			...
			animationChannelsOff.start();
		}
	}


	public void initializationsForTimeline() {
		toolbar.findViewById(R.id.filter_click_consumer).setOnClickListener(this);

		configureFilter(mainView);
		configureChannels(mainView);
		handleChannelsAdapter();
	}

	private void handleChannelsAdapter() {
		channelsAdapter = new ChannelsAdapter(channelsList, this);
		channelsAdapter.setHasStableIds(true);

		channelsLlm = new LinearLayoutManager(getContext(), RecyclerView.HORIZONTAL, false);

		feedChannels.setLayoutManager(channelsLlm);
		feedChannels.setAdapter(channelsAdapter);
	}
	...
	private void handleSorting(View view) {
		...
		// INFO: 3/25/19    no longer called here but @ the APPLY btn action
//		callTimeline(selectedSortOrder, true);
	...
	public void handleFilterToggle() {
		...
		if (filterVisible && filterContainer != null && filters != null && !mUser.isActingAsInterest()) {
			...
			if (index == -1 && hasFamilyRelations) {
				final FeedFilter family = new FeedFilter(Constants.CIRCLE_FAMILY_NAME);
				filters.add(1, family);
				final View view = LayoutInflater.from(getContext()).inflate(R.layout.layout_custom_checkbox_dark_filter, filterContainer, false);

				if (view != null) {
					((TextView) view.findViewById(R.id.text)).setText(family.getName());
	...
	public boolean hasChannelsVisible() {
		return feedChannels != null && feedChannels.getVisibility() == View.VISIBLE && channelsVisible;
	}
...
//		}
	...
	private void setChannels() {
		Pair<List<FeedChannel>, Integer> result = GFChannels.INSTANCE.getUpdatedChannels();
		List<FeedChannel> list = new ArrayList<>(result.getFirst());

		channelsList.clear();
		channelsList.addAll(list);
		channelsAdapter.notifyDataSetChanged();

		feedChannels.post(() -> channelsLlm.scrollToPosition(result.getSecond()));
	}

	@Override
	public void onChannelSelected(@org.jetbrains.annotations.Nullable String id) {
		GFChannels.INSTANCE.setCurrentSelectedChannel(id);
		callTimeline(true);
		handleChannelsVisibility();
	}

	@Override
	public void onChannelDeselected(@org.jetbrains.annotations.Nullable String id) {
		GFChannels.INSTANCE.setCurrentSelectedChannel(null);
		callTimeline(true);
		handleChannelsVisibility();
	}
	...
	//region - Subsection: DIARY/GLOBALSEARCH -
	...
	public Toolbar getToolbar() {
		return isTypeTimeline() ? toolbar : toolbarDiarySingle;
	}
