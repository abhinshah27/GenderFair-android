...
import com.groops.fairsquare.adapters.ChatMessagesAdapter
import com.groops.fairsquare.base.*
import com.groops.fairsquare.models.HLUser
import com.groops.fairsquare.models.chat.ChatMessage
import com.groops.fairsquare.models.chat.ChatMessageType
...
import com.groops.fairsquare.utility.helpers.ShareHelper
...
import kotlinx.android.synthetic.main.gf_toolbar_chat.view.*
...
class ChatMessagesFragment: HLFragment(), OnBackPressedListener, OnServerMessageReceivedListener,
    ...
    private val toolbarHeight by lazy { Utils.getToolbarHeight(context) }
...
//                                , { restoreScrollPosition() }, { restoreScrollPosition() }
    ...
    override fun configureLayout(view: View) {
        ...
        actionsBar.viewTreeObserver.addOnGlobalLayoutListener {
            ...
            else if (coords[1] > actionsBarY) {
                stopAnimating = true
                if (toolbarHeight > 0 && isScrollIdle)
                    showToolbar()
            }
            else stopAnimating = false
        ...
        btnSend.setOnClickListener {
            ...
            if (!participantId.isNullOrBlank() && chatRoom?.isValid() == true) {
                ...
                if (HLApp.canVibrate) vibrateForChat(context)
...
//        setMessages()
    ...
    override fun setLayout() {
        ...
        setStatusIndicatorAndString(
                chatRoom?.recipientStatus ?: ChatParticipant.Status.OFFLINE.value,
...
//            callServer(CallType.SET_READ, setIncomingRT = true)
    ...
    fun callServer(type: CallType, chatMessage: ChatMessage? = null, setIncomingRT: Boolean = false,
        ...
        when (type) {
            ...
            CallType.SET_READ -> {
                if (!chatRoomId.isNullOrBlank() &&
                        (setIncomingRT || (ChatRoom.getAmountUnreadMessagesByIdentityID(mUser.userId!!, realm) > 0))
                ) {
        ...
        HLRequestTracker.getInstance(activity?.application as? HLApp).handleCallResult(this, activity as? HLActivity, result, true, false)
    }
    ...
    private fun setStatusIndicatorAndString(status: Int, date: String? = null, dateObj: Date? = null) {
        with (status) {
            availabilityIndicator?.visibility = if (this == ChatParticipant.Status.ONLINE.value) View.VISIBLE else View.GONE
            participantLastSeen?.text =
                    if (this == ChatParticipant.Status.ONLINE.value)  {
    ...
    private fun callParticipant(callType: VoiceVideoCallType) {
        ...
        if (activity is HLActivity && chatRoom?.getParticipant() != null) {
            NotificationUtils.sendCallNotificationAndGoToActivity(
                    ...
                    calledIdentityUser = null,
                    calledIdentityParticipant = chatRoom?.getParticipant()!!,
                    callType = callType
            )
        }
    }
    ...
    // FIXME ??? : 4/1/19    can interest share content from chat?
