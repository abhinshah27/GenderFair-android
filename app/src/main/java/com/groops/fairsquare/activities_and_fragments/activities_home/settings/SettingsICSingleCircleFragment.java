...
import com.groops.fairsquare.adapters.SettingsSingleCircleAdapter;
...
public class SettingsICSingleCircleFragment extends HLFragment implements View.OnClickListener,
	...
	private String circleName, filter;
	...
	public static SettingsICSingleCircleFragment newInstance(@NonNull String circleName, @Nullable String filter) {
	...
	public void onSaveInstanceState(@NonNull Bundle outState) {
		...
		outState.putString(Constants.EXTRA_PARAM_1, circleName);
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		if (savedInstanceState != null) {
			...
			isInnerCircle = Utils.isStringValid(circleName) && circleName.equals(Constants.INNER_CIRCLE_NAME);
			isFamily = Utils.isStringValid(circleName) && circleName.equals(Constants.CIRCLE_FAMILY_NAME);
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.btn_add_new:
				settingsActivityListener.showInnerCircleSingleCircleFragment(Constants.INNER_CIRCLE_NAME, circleName);
				break;
		}
	}
		...
		if (object instanceof HLUserGeneric) {
			if (view.getId() == R.id.check) {
				...
				if (isInnerCircle && wantsFilter) {
					...
					bundle.putString("listID", circleName);
			...
			if (dialogActionsOnCircle != null) {
				...
				if (view != null && mType != null) {
					...
					if (mType == CallType.ADD_MEMBER) {
						...
						message = getString(R.string.settings_circles_remove_member_msg, userName, circleName);
			...
			case Constants.SERVER_OP_SETTINGS_IC_MEMBER_ADD_DELETE:
				...
				realm.executeTransaction(realm -> {
					if (operationId == Constants.SERVER_OP_SETTINGS_IC_MEMBER_ADD_DELETE) {
						HLCircle circle = new HLCircle(isInnerCircle ? filter : circleName);
...
//										mUser.getSelectedFeedFilters().remove(FeedFilterTypeEnum.getCallValue(Constants.INNER_CIRCLE_NAME));
	...
	protected void setLayout() {
		...
		titleCircle.setText(isInnerCircle ? circleName : getString(R.string.settings_title_circle, circleName));
		...
		searchBox.setText(query);
		searchBox.setHint(getString(R.string.settings_ic_search_box_hint, circleName));
	}
		...
		try {
			...
			if (type == CallType.GET) {
				if (isInnerCircle) {
					...
					result = HLServerCalls.getCircle(mUser.getUserId(), circleName, ++pageIdToCall);
			}
