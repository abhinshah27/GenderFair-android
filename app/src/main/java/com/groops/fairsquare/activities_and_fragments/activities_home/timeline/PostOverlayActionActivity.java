...
import com.groops.fairsquare.adapters.SharePeopleAdapter;
...
import com.groops.fairsquare.utility.helpers.BackgroundHelper;
...
//				}).start();
	...
	private void setCommentSectionAndOpen(boolean open) {
		if (open) {
			...
			MediaHelper.loadProfilePictureWithPlaceholder(this, mUser.getAvatarURL(), authorProfilePic, mUser.isActingAsInterest());
			...
			if (circlesList != null && !circlesList.isEmpty()) {
				queryResults.addAll(Stream.of(circlesList).filter(new Predicate<HLCircle>() {
					...
					public boolean test(HLCircle circle) {
						return circle.getName().toLowerCase().contains(query);
					}
