...
import com.groops.fairsquare.adapters.ChatCreationAdapterDiff
import com.groops.fairsquare.adapters.ChatParticipantDiffCallback
...
import kotlinx.android.synthetic.main.gf_toolbar_simple_back.view.*
import kotlinx.android.synthetic.main.layout_search_plus_rv.view.*
import org.json.JSONArray
import java.util.*

class ChatCreationFragment:
        HLFragment(), OnServerMessageReceivedListener, OnMissingConnectionListener,
    ...
    var loadedData = mutableListOf<ChatParticipant>()
    private var adapter: ChatCreationAdapterDiff? = null

    private var chatRoom: ChatRoom? = null
    private var currentContact: ChatParticipant? = null
    ...
    override fun configureLayout(view: View) {
        ...
        toolbar.back_arrow.rotation = 270f
        toolbar.back_arrow.setOnClickListener { activity?.onBackPressed() }
        ...
        if (adapter == null)
            adapter = ChatCreationAdapterDiff(ChatParticipantDiffCallback(), this).apply { this.setHasStableIds(true); }
    ...
    private fun setData(responseObject: JSONArray? = null) {
        ...
        PopulateChatHandler(responseObject).start()
    }
    ...
    override fun onChatClicked(user: ChatParticipant?) {
        if (user != null) {
            currentContact = user
    ...
    override fun onCallClicked(user: ChatParticipant?, callType: VoiceVideoCallType) {
        if (Utils.isContextValid(activity) && user != null) {
            currentContact = user
            NotificationUtils.sendCallNotificationAndGoToActivity(activity as HLActivity, mUser.userId, mUser.completeName, calledIdentityUser = null, calledIdentityParticipant = user, callType = callType)
        }
        else
    ...
    inner class PopulateChatHandler(response: JSONArray?):
            ComputeAndPopulateHandlerThread("populateChatCreation", response) {

        override fun customActions(jsonResponse: JSONArray?) {
            val users = jsonResponse?.optJSONObject(0)
            val list = users?.optJSONArray("users")
            loadedData.clear()
            if (list != null && list.length() > 0) {
                for (i in 0 until list.length()) {
                    val user = ChatParticipant.getParticipant(list.optJSONObject(i))
                    loadedData.add(user)
                }
                loadedData.sort()
            }
