...
import com.groops.fairsquare.activities_and_fragments.activities_home.global_search.GlobalSearchFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.profile.ProfileActivity;
import com.groops.fairsquare.activities_and_fragments.activities_home.profile.ProfileFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.profile.ProfileHelper;
import com.groops.fairsquare.activities_and_fragments.activities_home.timeline.LandscapeMediaActivity;
...
public class HomeActivity extends HLActivity implements View.OnClickListener, BasicInteractionListener,
		...
		ChatRoomsFragment.OnIdentitySwitchedListener {
...
//		fullScreenListener.setToolbarExpHeight(toolbar.getHeight());
	...
	protected void onResume() {
		...
		if (notificationHelper != null)
			notificationHelper.onResumeFromHomeActivity();
			...
			case Constants.RESULT_SELECT_IDENTITY:
				if (currentPagerItem == PAGER_ITEM_PROFILE && weakProfile != null) {
					...
					if (frg != null && frg.isVisible()) {
						frg.setViewCreated(true);
						frg.setProfileType(mUser.getProfileType());
						frg.clearImageResources();
...
//	}
	...
	public boolean closePostSheet() {
		if (bottomSheetHelper != null)
			return bottomSheetHelper.closePostSheet();
...
//				intent.putExtra(Constants.EXTRA_PARAM_1, PAGER_ITEM_NOTIFICATIONS);
			...
			if (newIntent) {
				...
				if (weakTimeline != null && weakTimeline.get() != null) {
					...
					weakTimeline.get().initializationsForTimeline();
				}
...
//	}
	...
	public View getTopIconNotificationDot() {
		if (weakProfile != null && weakProfile.get() != null)
			return weakProfile.get().getTopIconNotificationDot();
		return null;
	}
...
//						setBottomBar(true, position);
	...
	private void handleChatToReadDot(int pageSelected) {
		if (pageSelected == PAGER_ITEM_CHATS) {
			...
			if (mUser.isValid() && (ChatRoom.Companion.getAmountUnreadMessagesByIdentityID(mUser.getUserId(), realm) > 0))
	...
	@Override
	public void onIdentitySwitched(@NotNull String identityID) {
		if (weakProfile != null && weakProfile.get() != null) {
			weakProfile.get().setProfileType(identityID.equals(mUser.getUserId()) ? ProfileHelper.ProfileType.ME : ProfileHelper.ProfileType.INTEREST_ME);
			weakProfile.get().setObjectId(identityID);
			weakProfile.get().onResume();
		}
	}
