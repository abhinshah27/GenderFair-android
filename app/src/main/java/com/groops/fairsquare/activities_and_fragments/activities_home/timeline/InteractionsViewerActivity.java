...
import com.groops.fairsquare.adapters.InteractionCommentsAdapter;
import com.groops.fairsquare.adapters.InteractionHeartsAdapter;
import com.groops.fairsquare.adapters.InteractionSharesAdapter;
...
import com.groops.fairsquare.utility.SlidingPanelHandler;
...
public class InteractionsViewerActivity extends HLActivity implements View.OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ivHearts.setImageResource((mPost.getHeartsLeft() != null && mPost.getHeartsLeft() > 0) ?
				R.drawable.transition_hearts_orange : R.drawable.transition_hearts);
	...
	public int getLastPageId() {
		if (hUp)
			return HLApp.pageIdHearts;
		else if (cUp)
			return HLApp.pageIdComments;
		else if (sUp)
			return HLApp.pageIdShares;
...
//			else
//				colorRes = mPost.isYouLeftComments() ? R.color.gf_red_alpha_med : R.color.white_50;
					...
					if (heartsList != null) {
						...
						if (!background) {
							heartsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (HLApp.pageIdHearts - 1)), newItemsCount
							);
						}
					}
				}
...
//							commentsList.addAll(HLPosts.getInstance().getVisibleCommentsForPost(mPostId));
						if (!background) {
							commentsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (HLApp.pageIdComments) - 1), newItemsCount
							);
						}
					}
				}
					...
					if (sharesList != null) {
						...
						if (!background) {
							sharesAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (HLApp.pageIdShares - 1)), newItemsCount
							);
						}
					}
				}
...
//				cntSharesPost.setSelected(mPost.isYouDidShares());
	...
	private void call(boolean usePageId) {
		...
		switch (viewHitResId) {
			case R.id.button_hearts:
				type = InteractionPost.Type.HEARTS;
				if (usePageId) pageId = ++HLApp.pageIdHearts;
				else pageId = -1;
				break;

			case R.id.button_comments:
				type = InteractionPost.Type.COMMENT;
				if (usePageId) pageId = ++HLApp.pageIdComments;
				else pageId = -1;
				break;

			case R.id.button_shares:
				type = InteractionPost.Type.SHARE;
				if (usePageId) pageId = ++HLApp.pageIdShares;
				else pageId = -1;
				break;
		}
...
//						sUp = false;
	...
	private void handleTransitionDrawables(boolean first) {
		...
		@ColorRes int colorInactiveOrange = R.color.gf_red_alpha_med;
