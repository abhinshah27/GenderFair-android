...
import com.groops.fairsquare.activities_and_fragments.activities_home.profile.UserDetailFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsAccountFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsCirclesFoldersFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsICSingleCircleFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsICTimelineFeedFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsPrivacySelectionFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsSecurityLegacy2StepFragment;
import com.groops.fairsquare.activities_and_fragments.activities_home.settings.SettingsSecurityLegacyContactTriggerFragment;
import com.groops.fairsquare.activities_and_fragments.activities_login.LoginActivity;
import com.groops.fairsquare.activities_and_fragments.activities_onboarding.OnBoardingButtonsFragment;
import com.groops.fairsquare.base.HLActivity;
import com.groops.fairsquare.base.HLApp;
import com.groops.fairsquare.models.FamilyRelationship;
import com.groops.fairsquare.models.GenericUserFamilyRels;
import com.groops.fairsquare.models.HLCircle;
import com.groops.fairsquare.models.HLInterestAbout;
import com.groops.fairsquare.models.HLNotifications;
import com.groops.fairsquare.models.HLUser;
import com.groops.fairsquare.models.HLUserAboutMe;
import com.groops.fairsquare.models.HLUserAboutMeMore;
import com.groops.fairsquare.models.HLUserGeneric;
import com.groops.fairsquare.models.InteractionComment;
import com.groops.fairsquare.models.InteractionHeart;
import com.groops.fairsquare.models.InteractionPost;
import com.groops.fairsquare.models.InteractionShare;
import com.groops.fairsquare.models.LifeEvent;
import com.groops.fairsquare.models.MarketPlace;
import com.groops.fairsquare.models.MemoryMediaObject;
import com.groops.fairsquare.models.MoreInfoObject;
...
import com.groops.fairsquare.models.WishEmail;
import com.groops.fairsquare.models.WishListElement;
...
import com.groops.fairsquare.models.enums.UnBlockUserEnum;
...
public class HLServerCalls {
	...
	protected static String[] buildMessageJson(@NonNull String name, int opCode, @NonNull String dbName,
		...
		event.put("sessionID", SessionWrapper.isValid() ? SessionWrapper.INSTANCE.getSessionID() : "");
	...
	public static Object[] subscribeToSocket(final Context context, final String userId, boolean isChat) {
		...
		// INFO: 2019-05-30    if not connected socket subscription is always the FIRST call automatically performed
//		if (isChat) {
//			if (!HLApp.getSocketConnection().isConnectedChat())
//				return resultsNotConnected();
//		}
//		else {
//			if (!HLApp.getSocketConnection().isConnected())
//				return resultsNotConnected();
//		}
		...
		if (Utils.isStringValid(userId)) {
			...
			if (isChat) {
				if (!HLApp.subscribedToSocketChat) {
					HLApp.getSocketConnection().sendMessageChat(jsonResult[0]);
					HLApp.subscribedToSocketChat = true;
				}
			}
			else {
				if (!HLApp.subscribedToSocket) {
					HLApp.getSocketConnection().sendMessage(jsonResult[0]);
	...
	public static Object[] signUpOnBoarding(Context context, String userID) throws JSONException {
		...
		if (Utils.isStringValid(userID)) {
			...
			String sLoc = Locale.getDefault().toString();
			if (sLoc.length() >= 5)
				pairs.add(new Pair<>("language", sLoc.substring(0, 5)));
			else
				pairs.add(new Pair<>("language", Locale.getDefault().getLanguage()));
	...
	public static Object[] renewSession(String userId, String sessionId) throws JSONException {

//		if (!HLApp.getSocketConnection().isConnected())
//			return resultsNotConnected();

		LogUtils.d("RENEW_SESSION", "CALLING SERVER");

		String[] jsonResult = new String[2];
		List<List<Pair<String, Object>>> list;
		if (Utils.areStringsValid(userId, sessionId)) {
			list = new ArrayList<>();

			List<Pair<String, Object>> pairs = new ArrayList<>();
			pairs.add(new Pair<>("userID", userId));
			pairs.add(new Pair<>("sessionID", sessionId));

			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_OP_RENEW_SESSION,
				Constants.SERVER_CODE_COLL_USERS, list, "RENEW SESSION call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}

		return new Object[] { HLApp.getSocketConnection().isConnected(), jsonResult[0], jsonResult[1] };
	}
	...
	public static Object[] getTimelineForChannel(@NonNull String channelId, String channelName, int skip) throws JSONException {
		if (!HLApp.getSocketConnection().isConnected())
			return resultsNotConnected();
		...
		if (Utils.isStringValid(channelId)) {
			...
			List<Pair<String, Object>> pairs = new ArrayList<>();
			pairs.add(new Pair<>("userID", channelId));
			pairs.add(new Pair<>("name", channelName));
			pairs.add(new Pair<>("skip", skip));

			// INFO: 4/1/19    "pageID" kept for legacy purposes
			pairs.add(new Pair<>("pageID", 1));

			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_OP_GET_TIMELINE_CHANNEL,
					Constants.SERVER_CODE_COLL_POSTS, list, "GET TIMELINE FOR CHANNEL call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
...
//	}
	 ...
	 * {@link com.groops.fairsquare.activities_and_fragments.activities_home.profile.ProfileHelper.ProfileType#FRIEND}.
	 * @param interestIdToFetch must always refer to the {@link com.groops.fairsquare.models.Interest} to be fetched.
...
//			return resultsNotConnected();
	...
	public static Object[] updateProfile(@NonNull String userId, @NonNull String context,
		...
		return updateProfile(userId, context, aboutMe, lifeEvents, moreAboutMe, null, null, null,
				null, null);
	}
	...
	public static Object[] updateInterestProfile(@NonNull String userId, @NonNull String interestId,
		...
		return updateProfile(userId, null, null, null, null, interestId, interestName,
	...
	public static Object[] updateProfile(@NonNull String userId, @Nullable String context,
	                                     ...
	                                     @Nullable HLUserAboutMeMore moreAboutMe,
			...
			if (!Utils.isStringValid(context) && interestAbout != null) {
				...
				if (Utils.isStringValid(context)) {
					...
					if (context.equals(UserDetailFragment.LIFE_EVENTS) && lifeEvents != null) {
						...
						pairs.add(new Pair<>("sex", moreAboutMe.getGender()));
...
//			return resultsNotConnected();
	...
	public static Object[] getChannels() throws JSONException {

		String[] jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_GET_CHANNELS,
				Constants.SERVER_CODE_DB_NAME, Constants.SERVER_CODE_COLL_INTERESTS, new ArrayList<>(),
				"INTERESTS GET CHANNELS call");
		HLApp.getSocketConnection().sendMessage(jsonResult[0]);

		return new Object[] { HLApp.getSocketConnection().isConnected(), jsonResult[0], jsonResult[1] };
	}

	public static Object[] getCompaniesOrBrands(String companyID, boolean fromIndustry) throws JSONException {
		...
		if (Utils.isStringValid(companyID)) {
			list = new ArrayList<>();
			List<Pair<String, Object>> pairs = new ArrayList<>();
			pairs.add(new Pair<>(fromIndustry ? "industryID" : "pcID", companyID));
			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_GET_BRANDS,
				Constants.SERVER_CODE_DB_NAME, Constants.SERVER_CODE_COLL_INTERESTS, list,
				"INTERESTS GET BRANDS call");
		}
...
//			return resultsNotConnected();
	...
	public static Object[] logout(@NonNull Context context, String userId) throws JSONException {
...
//			return resultsNotConnected();
	...
	public static Object[] settingsOperationsOnCircles(@NonNull String userId, @NonNull Bundle bundle,
	                                                   SettingsCirclesFoldersFragment.CallType type)
			throws JSONException {
		...
		if (Utils.isStringValid(userId)) {
			...
			if (type == SettingsCirclesFoldersFragment.CallType.RENAME) {
				opId = Constants.SERVER_OP_SETTINGS_IC_CIRCLES_RENAME;
				tag = "RENAME CIRCLE";
			}
			else if (type == SettingsCirclesFoldersFragment.CallType.ADD) {
				tag = "ADD CIRCLE";
			}
			else if (type == SettingsCirclesFoldersFragment.CallType.REMOVE) {
				tag = "DELETE CIRCLE";
			}
	...
	public static Object[] settingsOperationsOnFeedSettings(@NonNull String userId, @NonNull Bundle bundle,
		...
		if (!HLApp.getSocketConnection().isConnected() && type == SettingsICTimelineFeedFragment.CallType.SET)
			return resultsNotConnected();
	...
	public static Object[] settingsOperationsOnPrivacy(@NonNull String userId, int idItem,
		...
		if (!HLApp.getSocketConnection().isConnected() && type == SettingsPrivacySelectionFragment.CallType.SET)
			return resultsNotConnected();
...
//				if (Utils.isStringValid(queryString))
	...
	public static Object[] validateSignUpEmail(String email, String password)
			throws JSONException {
		...
		if (Utils.areStringsValid(email, password)) {
			...
			jsonResult = buildMessageJson(Constants.SERVER_OP_VALIDATE_EMAIL,
					Constants.SERVER_CODE_COLL_USERS, list, "VALIDATE EMAIL call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
	...
	public static Object[] callButtons(@NonNull OnBoardingButtonsFragment.ButtonType type)
			throws JSONException {
...
//			return resultsNotConnected();
		...
		String[] jsonResult = buildMessageJson(
				(type == OnBoardingButtonsFragment.ButtonType.CHANNELS) ? Constants.SERVER_OP_ONBOARDING_GET_CHANNELS : Constants.SERVER_OP_ONBOARDING_GET_INTERESTS,
				Constants.SERVER_CODE_COLL_USERS,
				list,
				(type == OnBoardingButtonsFragment.ButtonType.CHANNELS) ? "ON-BOARDING GET CHANNELS call" : "ON-BOARDING GET INTERESTS call");
		HLApp.getSocketConnection().sendMessage(jsonResult[0]);

		return new Object[] { HLApp.getSocketConnection().isConnected(), jsonResult[0], jsonResult[1] };
	}

	public static Object[] sendSelectedElements(String userID, @NonNull OnBoardingButtonsFragment.ButtonType type, Set<String> elements)
			throws JSONException {
		...
		if (Utils.isStringValid(userID)) {
			...
			JSONArray jelements = new JSONArray(elements);
			pairs.add(new Pair<>(
					(type == OnBoardingButtonsFragment.ButtonType.CHANNELS) ? "channels" : "interests",
					jelements)
			);
			list.add(pairs);

			jsonResult = buildMessageJson(
					(type == OnBoardingButtonsFragment.ButtonType.CHANNELS) ?
							Constants.SERVER_OP_ONBOARDING_STEP2 : Constants.SERVER_OP_ONBOARDING_STEP4,
					Constants.SERVER_CODE_COLL_USERS,
					list,
					(type == OnBoardingButtonsFragment.ButtonType.CHANNELS) ?
							"ON-BOARDING SEND CHANNELS call" : "ON-BOARDING SEND INTERESTS call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
	...
	public static Object[] sendUserNames(String userID, String firstName, String lastName) throws JSONException {
		...
		String[] jsonResult = new String[2];
		List<List<Pair<String, Object>>> list;
		if (Utils.areStringsValid(userID, firstName)) {
			list = new ArrayList<>();

			List<Pair<String, Object>> pairs = new ArrayList<>();
			pairs.add(new Pair<>("userID", userID));
			pairs.add(new Pair<>("firstName", firstName));
			pairs.add(new Pair<>("lastName", (lastName == null) ? "" : lastName));
			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_OP_ONBOARDING_STEP3,
					Constants.SERVER_CODE_COLL_USERS, list, "ON-BOARDING SEND NAMES call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
	...
	public static void doActionOnNotification(HLActivity activity, String notificationId, ActionTypeEnum type,
		...
		HLRequestTracker.getInstance(((HLApp) activity.getApplication())).handleCallResult(connectionListener, activity, result);
	}
...
//			return resultsNotConnected();
	...
	public static Object[] getMenuIndustries() throws JSONException {

//		if (!HLApp.getSocketConnection().isConnected())
//			return resultsNotConnected();

		String[] jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_GET_INDUSTRIES,
				Constants.SERVER_CODE_DB_NAME, Constants.SERVER_CODE_COLL_INTERESTS, new ArrayList<>(),
				"MENU GET LANDING ITEMS call");
	...
	public static Object[] getMenuCompanies(String industryID, int skip) throws JSONException {
...
//			return resultsNotConnected();
		...
		if (Utils.isStringValid(industryID) && skip >= 0) {
			pairs.add(new Pair<>("industryID", industryID));
			pairs.add(new Pair<>("skip", skip));
			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_GET_COMPANIES,
					Constants.SERVER_CODE_DB_NAME, list, "MENU GET COMPANIES call");
			HLApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
