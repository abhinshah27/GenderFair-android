 ...
 * a dynamic key is added and it is the [Session.sessionID]. This is used to encode and decode all
 * the authenticated calls.
 *
 * For the non-authenticated ones (1000, 1005, 1010, 1012, 1416, 1422, 1514, 4000, 4001, 4002, 4003, 4004) the static
 * runtime retrieved key is used.
 *
 */
object Crypto {
    ...
    private fun retrieveKey(): ByteArray {
        return if (SessionWrapper.isValid()) {
            LogUtils.d(LOG_TAG, "Used DYNAMIC KEY: ${SessionWrapper.sessionID}")
            SessionWrapper.sessionID!!.toByteArray()
        } else {

            LogUtils.d(LOG_TAG, "Used STATIC KEY")
