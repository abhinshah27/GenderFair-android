...
public class HLRequestTracker {
    ...
    private synchronized void storeId(String body, String idOp, HLActivity caller, boolean isChat) {
        if (!Utils.areStringsValid(idOp, body))
            return;

        Map<String, Request> map = isChat ? requestsChat : requests;
        map.put(idOp, new Request(idOp, body, isChat, caller));
    }
        ...
        try {
            if (!map.isEmpty()) {
                for (Map.Entry<String, Request> entry : map.entrySet()) {
                    ...
                    if (r.isValid()) {
                        if (r.isChat())
                            HLApp.getSocketConnection().sendMessageChat(r.getCorrectBody());
                        else
                            HLApp.getSocketConnection().sendMessage(r.getCorrectBody());
                    }
                    else
                        map.remove(r.getId());
                }
    ...
    private void onDataReceived(String data) throws JSONException {
        ...
        if (result != null && isError(result)) {
            ...
            LogUtils.d("RENEW_SESSION", operationId + " IS ERROR: " + errorCode);

            Request req = getRequest(operationId);
            if (!SessionHandler.INSTANCE.isSessionAvailable(application, errorCode, errorDescription, req)) {
                LogUtils.d("RENEW_SESSION", operationId + " RETURNING");
                return;
            }
        }
            ...
            if (application != null) {
                ...
                if (action == Constants.SERVER_OP_GET_CIRCLE && Utils.isStringValid(operationId)) {
                    ...
                    switch (action) {
                        ...
                        case Constants.SERVER_OP_RT_PUSH_CHANNELS:
...
//				application.getHLContext().sendBroadcast(intent);
    ...
    public void handleCallResult(HLActivity activity, Object[] results, boolean isChat) {
        if (results != null) {
    ...
    public void handleCallResult(OnMissingConnectionListener listener, HLActivity activity, Object[] results, boolean isChat, boolean wantsProgress) {
        if (results != null) {
            ...
            if (!((boolean) results[0]) && Utils.isContextValid(activity)) {
                ...
                if (!HLApp.justComeFromBackground)
