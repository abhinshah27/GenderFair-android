...
import com.groops.fairsquare.voiceVideoCalls.CallsActivityNoService;
...
import com.groops.fairsquare.websocket_connection.HLWebSocketClient;
import com.groops.fairsquare.websocket_connection.OnConnectionChangedListener;
...
import com.groops.fairsquare.websocket_connection.session.SessionObserver;
import com.groops.fairsquare.websocket_connection.session.SessionState;
import com.groops.fairsquare.websocket_connection.session.SessionSubject;
...
public abstract class HLActivity extends AppCompatActivity implements OnServerMessageReceivedListener,
		OnConnectionChangedListener, Handler.Callback, SessionObserver {
...
//		}
	...
	protected void onStart() {
		...
		HLSocketConnection connection = HLApp.getSocketConnection();
		if (connection != null) {
...
//			unregisterReceiver(serverMessageReceiver);
	...
	public void handleSuccessResponse(int operationId, JSONArray responseObject) {
		switch (operationId) {
			case Constants.SERVER_OP_RENEW_SESSION:
				handleNewSession(responseObject);
				break;
		}
	}

	private void handleNewSession(JSONArray responseObject) {
		if (responseObject == null || responseObject.length() == 0) return;

		try {
			JSONObject json = responseObject.getJSONObject(0);
			SessionWrapper.updateSessionID(json.optString("sessionID"));
		}
		...
		if (genericProgressIndicator != null) {
			genericProgressIndicator.animate().setDuration(100).alpha(
				...
				public void onAnimationStart(Animator animation) {
					if (show) genericProgressIndicator.setVisibility(View.VISIBLE);
				}

				@Override
				public void onAnimationEnd(Animator animation) {
					if (!show) genericProgressIndicator.setVisibility(View.GONE);
				}
			});
		}
	}
	...
	public void openProgress() {
		runOnUiThread(() -> showProgressIndicator(true));
	}
...
//				|
	...
	//region == Session Observer ==

	private void subscribeToSessionSubject() {
		SessionSubject.INSTANCE.attach(this);
	}

	@Override
	public void onSessionUpdated(@NotNull SessionState sessionState, @Nullable String errorDescription, @NotNull Object[] serverRequest) {

		HLRequestTracker.getInstance((OnApplicationContextNeeded) getApplication())
				.handleCallResult(this, serverRequest, false);

		LogUtils.d("RENEW_SESSION", "SOCKET CONNECTED: " + serverRequest[0]);

		if (sessionState == SessionState.FORCE_LOGOUT)
			Utils.logOut(this, null, errorDescription);
	}
